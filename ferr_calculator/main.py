...
options = ["FERR", "CELI", "Non Enregistré"]

selection = st.segmented_control(
    label="Produit", options=options, default="FERR"
)

with st.sidebar:
    st.image("ferr_calculator/static/1743878344560-26372b5e-087c-4937-b368-72e4aa19caae_1.jpg", width=500)
    st.title(f"Versement de {selection}")

    start_value = st.number_input(f"Valeur du {selection}", value=200_000)
    start_age = st.number_input(f"Début du {selection} à l'âge de", value=71, min_value=50)
    end_age = st.number_input(f"Fin du {selection} à l'âge de", value=90, max_value=100)
...
fig = px.line(
    ...
    title=f"Valeur du {selection} en fonction du temps"
)
